...
def deepget(obj: Any, path: str, default: Any = None) -> Any:
    """
    Safely get a nested value from a dictionary or list using dot notation.
    Supports array indices (e.g., "edges.0.node") via numeric string keys.

    Args:
        obj: The dictionary or list to traverse
        path: Dot-separated path (e.g., "comet_sections.content.story")
    ...
    Examples:
        deepget(node, "comet_sections.content.story.attachments")
        deepget(media, "videoDeliveryLegacyFields.browser_native_hd_url")
        deepget(raw_node, "timeline_list_feed_units.edges.0.node.permalink_url")
    """
    if obj is None:
        return default
    ...
    for key in keys:
        # Handle array index (numeric string like "0", "1")
...
def find_message_text(data):
    # $.raw_data[0].data.node.timeline_list_feed_units.edges[0].node.comet_sections.content.story.comet_sections.message.story.message.text
    return data["data"]["node"]["timeline_list_feed_units"]["edges"][0]["node"][
        "comet_sections"
...
def find_creation(data):
    # data.node.timeline_list_feed_units.edges[0].node.comet_sections.context_layout.story.comet_sections.metadata[0].story.creation_time
    return data["data"]["node"]["timeline_list_feed_units"]["edges"][0]["node"][
        "comet_sections"
...
def find_owning_profile(data):
    # data.node.timeline_list_feed_units.edges[0].node.feedback.owning_profile
    return data["data"]["node"]["timeline_list_feed_units"]["edges"][0]["node"][
        "feedback"
    ]["owning_profile"]
...
def get_current_time(timezone="Asia/Taipei"):
    ...
    target_current_time = current_time_utc.replace(tzinfo=pytz.utc).astimezone(
        target_timezone
    )
...
def get_payload(doc_id_in: str, id_in: str, before_time: str = None, cursor: str = None):
    variables_dict = {
        "afterTime": None,
...
def get_before_time(time_zone="Asia/Ho_Chi_Minh"):
